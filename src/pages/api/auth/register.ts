...
export const POST: APIRoute = async ({ request, cookies, redirect }) => {
  ...
  async function loginUser(email, password) {
    const url = 'https://interview-plus.onrender.com/api/register';
    const data = {
      name: name,
      email: email,
      password: password
  ...
  if (globalToken) {
    ...
    cookies.set('sb-access-name', name, { sameSite: 'strict', path: '/', secure: false });    
    // Redirect to the protected page
    return redirect("/api/auth/protected");
  } else {
    return new Response("Register failed", { status: 401 });
  }
};
